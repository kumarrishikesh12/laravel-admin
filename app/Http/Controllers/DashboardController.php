...
*/
    ...
    /* -------------------------------- FETCH TWITTER DATA ------------------------------------ */
      ...
      $twitter_hashtags = DB::select( DB::raw("SELECT hashtags FROM userssocial_credentials WHERE user_id = '$user->id' and social_webname = '$twitter_webname' ") );
        ...
        if(empty($twitter_hashtags)){
          ...
          $hashtags_data = $twitter_hashtags['0']->hashtags; 
            ...
            if(!empty($hashtags_data)){
               ...
               $hashtag_search = $twitter_hashtags['0']->hashtags; //get value in hashtag_search variable
             ...
             $twitter_data = UsersSocialCredentials::where('social_webname','=',$twitter_social_webname)->Where('user_id',$user_id)->first(); //Twitter_data
               ...
               //$hashtags = '#india_india'; //static_hastag
                ...
                if(isset($next_results) && !empty($next_results) && isset($max_id) && !empty($max_id)){
                      ...
                      $requestMethod = 'GET';
                      $twitter = new TwitterAPIExchange($settings);
                      $tweest_json_next_page = $twitter->setGetfield($getfield)->buildOauth($url, $requestMethod)->performRequest();
        ...
        if(!empty($instas) && !empty($insta_array) && !empty($next_page) && !empty($tags_json_link) && !empty($tweets) && !empty($tweest_json) && !empty($tweets_next_page) && !empty($tweest_json_next_page) ) {
            
            #both data exist...
            return view('all_feeds')->with('tweets',$tweets)->with('tweest_json',$tweest_json)->with('tweets_next_page',$tweets_next_page)->with('tweest_json_next_page',$tweest_json_next_page)->with('instas',$instas)->with('insta_array',$insta_array)->with('next_page',$next_page)->with('tags_json_link',$tags_json_link);
