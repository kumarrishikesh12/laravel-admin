...
@section('title')
    {!! trans('admin/product_attributes.productattributes') !!}
@stop
@section('content')
<!-- BEGIN .main-heading -->
        ...
        <div class="row">
            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4">
                ...
                <div class="page-title">
                    <h3>{!! trans('admin/product_attributes.productattributes') !!}</h3>
                </div>
            </div>
            <div class="col-xl-8 col-lg-8 col-md-8 col-sm-8">
                <div class="daterange-container">
                    <a data-toggle="tooltip" title="{{ trans('admin/common.create') }}" class="btn btn-primary btn-sm btn-create" href="{{ url('admin/productattributes/create') }}"><i class="icon-plus"></i> {{ trans('admin/common.create') }}</a>
                    ...
                    <table id="data-table" class="table table-hover table-striped">
                        <thead class="text-uppercase">
                            <tr>
                                ...
                                <th>{{ trans('admin/product_attributes.name') }}</th>
                                <th>{{ trans('admin/product_attributes.marketplace') }}</th>
                                <th>{{ trans('admin/product_attributes.attribute') }}</th>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $(".modal").on("show.bs.modal", function(e) {
                ...
                var url = "{{ url('admin/productattributes') }}/"+target+id;
            ...
            var table = $('#data-table').on( 'init.dt', function () {     
                    ...
                    "order": [[ 1, "asc" ]],
                    "ajax": {
                        "url": "{{ url('admin/productattributes/data') }}",
                        "type": 'POST',
                        "headers": {
                    ...
                    "columns": [
                        { "data": "id" },
                        { "data": "name" },
                        { "data": "marketplace_name", "name": "marketplace_id" },
                        { "data": "attribute_name", "name": "attribute_id" },
                        { "data": "created_at" },
                ...
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if(result===true){
                            ...
                            $.ajax({
                                "url": "{{ url('admin/productattributes/destroy') }}",
                                "type": "POST",
                                "headers": {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                },
            ...
            $(document).delegate('.btn-status', 'click', function() { 
                ...
                $(button).tooltip("hide");
                $.ajax({
                    "url": "{{ url('admin/productattributes') }}/"+id,
                    "type": "POST",
                    "headers": {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
